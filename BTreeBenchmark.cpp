...
void run_benchmark(size_t dataSize) {
    using key_type = int;
    using TupleT = db::Tuple;
    using Buffer = BufferPool<POOL_SIZE>;
    using SortPolicy = OptimizedBTree<key_type, LeafNode, BufferPool<POOL_SIZE>>::SortPolicy;
    ...
    for (double sortedness : sortedness_levels) {
        for (double read_ratio : read_ratios) {
            ...
            {
                Buffer pool;
                SimpleBPlusTree<key_type, BufferPool<POOL_SIZE>> tree(pool, "simple", td, 0);
            ...
            {
                Buffer pool;
                OptimizedBTree<key_type, LeafNode, BufferPool<POOL_SIZE>> tree(pool, SortPolicy::SORT_ON_SPLIT, 0, "opt", td);
                ...
                results.push_back({"OptimizedBTree", sortedness, read_ratio, insert_time, search_time, 0, tree.get_fast_path_hits()});
            }

            // === Benchmark 3: OptimizedBTree with LeafNodeLSM ===
            {
                Buffer pool;
                OptimizedBTree<key_type, LeafNodeLSM, BufferPool<POOL_SIZE>> tree(pool, OptimizedBTree<int, LeafNodeLSM, BufferPool<64>>::SORT_ON_SPLIT, 0, "lsm", td);
                ...
                results.push_back({"LSMTree", sortedness, read_ratio, insert_time, search_time, 0, tree.get_fast_path_hits()});
            }
        }
    }
